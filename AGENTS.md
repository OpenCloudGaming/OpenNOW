...
- Login flow mechanics: the app opens NVIDIA OAuth in an **external browser** via `shell.openExternal()` and runs a temporary `localhost` callback server (ports in `src/main/gfn/auth.ts`) to receive the `?code=` redirect. For this to work in a headless/container desktop, a **default web browser must be configured** (e.g. `xdg-settings set default-web-browser google-chrome.desktop`) and `DISPLAY` must be set for the Electron process; otherwise sign-in fails with "Failed to execute default Web Browser" and the in-app login never completes. Because the callback server times out, do not reuse a stale OAuth browser window — start each login fresh from the app.

...
function buildSearchText(title: string, stores: string[], genres: string[], featureLabels: string[]): string { return [title, ...stores, ...genres, ...featureLabels].filter((value) => value.trim().length > 0).join(" ").toLowerCase(); }
function appToGame(app: AppData): GameInfo { const variants = app.variants?.map((variant) => ({ id: variant.id, store: variant.appStore, supportedControls: variant.supportedControls ?? [], librarySelected: variant.gfn?.library?.selected, libraryStatus: variant.gfn?.library?.status, lastPlayedDate: variant.gfn?.library?.lastPlayedDate })) ?? []; const selectedVariantIndex = app.variants?.findIndex((variant) => variant.gfn?.library?.selected === true) ?? -1; const safeIndex = selectedVariantIndex >= 0 ? selectedVariantIndex : 0; const selectedVariantId = variants[safeIndex]?.id; const fallbackNumericVariantId = variants.find((variant) => isNumericId(variant.id))?.id; const launchAppId = isNumericId(selectedVariantId) ? selectedVariantId : fallbackNumericVariantId ?? (isNumericId(app.id) ? app.id : undefined); const imageUrl = app.images?.GAME_BOX_ART ?? app.images?.TV_BANNER ?? app.images?.HERO_IMAGE ?? undefined; const genres = extractGenres(app); const featureLabels = extractFeatureLabels(app); const availableStores = [...new Set(variants.map((variant) => variant.store).filter((store) => typeof store === "string" && store.trim().length > 0))]; const lastPlayed = variants.map((variant) => variant.lastPlayedDate).find((value): value is string => typeof value === "string" && value.length > 0); return { id: app.id, uuid: app.id, launchAppId, title: app.title, description: app.description, longDescription: app.longDescription, featureLabels, genres, imageUrl: imageUrl ? optimizeImage(imageUrl) : undefined, playType: app.gfn?.playType, membershipTierLabel: app.gfn?.minimumMembershipTierLabel, selectedVariantIndex: Math.max(0, selectedVariantIndex), variants, availableStores, searchText: buildSearchText(app.title, availableStores, genres, featureLabels), isInLibrary: variants.some((variant) => variant.librarySelected || variant.libraryStatus === "IN_LIBRARY"), lastPlayed }; }

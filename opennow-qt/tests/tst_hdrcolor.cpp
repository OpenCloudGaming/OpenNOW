...
private slots:
    ...
    void chromeLayerPreservesColorAlphaAndSurfaceChanges()
    {
        ...
        const auto chromePixel = QPoint(16, 32) * image.devicePixelRatio();
        const auto backgroundPixel = QPoint(48, 32) * image.devicePixelRatio();
        QCOMPARE(image.pixelColor(chromePixel), QColor(Qt::red));
        QCOMPARE(image.pixelColor(backgroundPixel), QColor(Qt::blue));
        auto *chrome = root->findChild<QQuickItem *>(QStringLiteral("chrome"));
        QVERIFY(chrome);
        chrome->setProperty("color", QColor::fromRgbF(1, 0, 0, 0.5));
        QTRY_VERIFY((image = window.grabWindow()).pixelColor(chromePixel).blue() > 120);
        QVERIFY(std::abs(image.pixelColor(chromePixel).red() - 128) <= 2);
        QVERIFY(std::abs(image.pixelColor(chromePixel).blue() - 128) <= 2);
        window.showFullScreen();
        QTRY_COMPARE(window.visibility(), QWindow::FullScreen);
        QVERIFY(!window.grabWindow().isNull());
        window.showNormal();

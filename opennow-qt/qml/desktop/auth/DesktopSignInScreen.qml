...
FocusScope {
    ...
    Flickable {
        ...
        anchors.fill: parent
        contentWidth: width
        contentHeight: Math.max(height, signInLayout.implicitHeight + 48)
        ...
        Column {
            ...
            y: Math.max(24, (viewport.height - implicitHeight) / 2)
            width: Math.max(0, Math.min(root.wideLayout ? 1264 : 472, viewport.width - 48))
            spacing: 24

            Row {
                x: root.wideLayout ? 0 : (parent.width - width) / 2
                spacing: 11
                Image {
                ...
                Rectangle {
                    anchors.verticalCenter: parent.verticalCenter
                    width: betaLabel.implicitWidth + 14
                    height: 20
                    radius: 6
                    color: Qt.rgba(DesktopTokens.amber.r, DesktopTokens.amber.g, DesktopTokens.amber.b, 0.12)
                    Text {
                        id: betaLabel
                        anchors.centerIn: parent
                        text: qsTr("BETA")
            ...
            Item {
                ...
                Column {
                    ...
                    width: Math.max(0, Math.min(560, body.width - card.width - 64))
                    anchors.verticalCenter: parent.verticalCenter
                    spacing: 24
                    ...
                    Text {
                        width: parent.width
                        text: qsTr("OpenNOW is a native client for GeForce NOW and its alliance partners. Sign in with your provider to access your games.")
                        wrapMode: Text.WordWrap
                    ...
                    Row {
                        ...
                        Text {
                            ...
                            text: qsTr("OpenNOW 1.0.0 is a beta. You'll find more details after signing in.")
                            wrapMode: Text.WordWrap
                ...
                Rectangle {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    width: Math.min(472, parent.width)
                    height: idleColumn.implicitHeight + 52
                    radius: 22
                    color: Qt.rgba(DesktopTokens.shell.r, DesktopTokens.shell.g, DesktopTokens.shell.b, 0.88)
                    border.width: 1
                    border.color: DesktopTokens.seam
                    layer.enabled: true
                    layer.effect: MultiEffect {
                    ...
                    Rectangle {
                        id: providerMenu
                        x: idleColumn.x
                        y: idleColumn.y + providerButton.parent.y + providerButton.y + providerButton.height + 6

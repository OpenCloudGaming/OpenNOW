...
const APP_METADATA_QUERY_HASH: &str =
    "39187e85b6dcf60b7279a5f233288b0a8b69a8b1dbcfb5b25555afdcb988f0d7";
...
impl GfnApiClient {
    ...
    async fn fetch_panels(&self, panel_names: &[&str], vpc_id: &str) -> Result<Vec<Panel>> {
        let token = self.token().context("No access token for panel fetch")?;
        ...
        let extensions_str =
            serde_json::to_string(&extensions).context("Failed to serialize extensions")?;
        ...
        debug!(
            "Panel response (first 500 chars): {}",
            &body_text[..body_text.len().min(500)]
        );

        let graphql_response: GraphQLResponse =
            serde_json::from_str(&body_text).context(format!(
                "Failed to parse panel response: {}",
    ...
    fn app_to_game_info(app: AppData) -> GameInfo {
        ...
        let selected_variant_index = app
            .variants
            .as_ref()
            .and_then(|vars| {
                vars.iter().position(|v| {
                    v.gfn
                        .as_ref()
        ...
        let variant_id = selected_variant.map(|v| v.id.clone()).unwrap_or_default();
        ...
        GameInfo {
            ...
            membership_tier_label: app
                .gfn
                .as_ref()
                .and_then(|g| g.minimum_membership_tier_label.clone()),
            playability_text: app
                .gfn
                .as_ref()
                .and_then(|g| g.catalog_sku_strings.as_ref())
                .and_then(|s| s.sku_based_playability_text.clone()),
    ...
    pub async fn fetch_sectioned_games(&self, vpc_id: Option<&str>) -> Result<Vec<GameSection>> {
        ...
        for panel in panels {
            ...
            for section in panel.sections {
                ...
                let games: Vec<GameInfo> = section
                    .items
                    .into_iter()
                    .filter_map(|item| {
                        if let PanelItem::GameItem { app } = item {
    ...
    pub async fn fetch_public_games(&self) -> Result<Vec<GameInfo>> {
        ...
        let raw_games: Vec<RawGameInfo> =
            serde_json::from_str(&text).context("Failed to parse games JSON")?;

        let games: Vec<GameInfo> = raw_games
            .into_iter()
            .filter_map(|g| {
                let title = g.title?;
                ...
                let app_id = g.steam_url.as_ref().and_then(|url| {
                    url.split("/app/")
                        .nth(1)
    ...
    pub async fn fetch_app_details(&self, app_id: &str) -> Result<Option<GameInfo>> {
        let token = self.token().context("No access token for app details")?;
        ...
        info!(
            "Fetching app details for ID: {} (Variables: {})",
            app_id, variables_str
        );
        ...
        if !response.status().is_success() {
            ...
            error!(
                "App details failed for {}: {} - Body: {}",
                app_id, status, error_body
            );
        ...
        let response_data: AppMetaDataResponse =
            serde_json::from_str(&body).context("Failed to parse app details")?;
...
pub async fn fetch_server_info(access_token: Option<&str>) -> Result<ServerInfo> {
    ...
    let response = request.send().await.context("Server info request failed")?;
    ...
    let server_response: ServerInfoResponse = response
        .json()
        .await
        .context("Failed to parse server info")?;

    let vpc_id = server_response
        .request_status

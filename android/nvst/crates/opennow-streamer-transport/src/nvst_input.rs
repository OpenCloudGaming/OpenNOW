    ...
    pub(crate) fn encode(
        ...
        for event in events {
            ...
            match input_type {
                ...
                INPUT_TOUCH => {
                    let length = touch_event_length(event.bytes)?;
                    if length != event.bytes.len() {
                        return Err(NvstInputCodecError::Malformed("trailing touch data"));
                    }
                    encoded.push(remote_input_message(
                        remote_input_packet(INPUT_TOUCH, &event.bytes[4..]),
                        event.timestamp_us,
                    ));
                }
    ...
    while cursor < packet.len() {
        match packet[cursor] {
            0x22 => {
                ...
                let length = native_event_length(input_type, &packet[start..])?;
...
fn native_event_length(input_type: u32, bytes: &[u8]) -> Result<usize, NvstInputCodecError> {
    match input_type {
        ...
        INPUT_TEXT => Ok(bytes.len()),
        INPUT_TOUCH => touch_event_length(bytes),
        other => Err(NvstInputCodecError::UnsupportedType(other)),
    }
}

fn touch_event_length(bytes: &[u8]) -> Result<usize, NvstInputCodecError> {
    require_len(bytes, 8, "short touch header")?;
    let count = usize::from(read_u16_be(bytes, 6).unwrap());
    let length = 8 + 16 * count;
    if !(1..=40).contains(&count) || read_u16_be(bytes, 4) != Some(length as u16) {
        return Err(NvstInputCodecError::Malformed("invalid touch count/length"));
    }
    require_len(bytes, length, "truncated touch records")?;
    if bytes[8..length].chunks_exact(16).any(|touch| touch[0] >= 8) {
        return Err(NvstInputCodecError::Malformed("invalid touch slot"));
    }
    Ok(length)
}
...
fn text_messages(event: &NativeEvent<'_>) -> Result<Vec<NvstEncodedInput>, NvstInputCodecError> {
    // native_events already removed the 0x22 single-event marker. Only the four-byte
    // type remains before UTF-8; skipping five drops every individually typed character.
    require_len(event.bytes, 4, "short text packet")?;
    let text =
        std::str::from_utf8(&event.bytes[4..]).map_err(|_| NvstInputCodecError::UnsupportedText)?;
...
mod tests {
    ...
    #[test]
    fn touch_batches_preserve_contacts_and_release_over_reliable_control() {
        let mut touch = hex("1800000000280002");
        touch.extend_from_slice(&hex("00018000ffff03040000000000001000"));
        touch.extend_from_slice(&hex("07020000800000000000000000001001"));
        let mut wrapped = hex("23000000000000200022");
        wrapped.extend_from_slice(&touch);
        let mut codec = NvstInputCodec::default();
        let messages = codec.encode(&wrapped, 0).unwrap();
        assert_eq!(messages.len(), 1);
        assert_eq!(messages[0].route, NvstInputRoute::ControlReliable);
        // ServerControl 0206, outer RemoteInput 0e, then inner type 24.
        let packet = &messages[0].bytes;
        assert_eq!(&packet[..2], &0x0206_u16.to_le_bytes());
        assert_eq!(&packet[16..20], &24_u32.to_le_bytes());
        assert_eq!(&packet[20..56], &touch[4..]);
        assert_eq!(&packet[packet.len() - 8..], &0x2000_u64.to_le_bytes());
        let raw = codec.encode(&touch, 0x2000).unwrap();
        assert_eq!(raw, messages);
        for length in 0..touch.len() {
            assert!(codec.encode(&touch[..length], 0).is_err());
        }
        let mut bad = touch.clone();
        bad[7] = 41;
        assert!(codec.encode(&bad, 0).is_err());
        let mut bad = touch.clone();
        bad[8] = 8;
        assert!(codec.encode(&bad, 0).is_err());
        let mut trailing = touch;
        trailing.push(0);
        assert!(codec.encode(&trailing, 0).is_err());
    }

    #[test]
    fn single_typed_character_is_not_discarded() {
        // Android InputEncoder: single-event marker, little-endian type 23, UTF-8 text.
        let packet = [0x22, 23, 0, 0, 0, b'a'];
        let messages = NvstInputCodec::default().encode(&packet, 17).unwrap();
        assert_eq!(messages.len(), 2);
        assert_eq!(&messages[0].bytes[16..20], &INPUT_KEY_DOWN.to_le_bytes());
        assert_eq!(&messages[1].bytes[16..20], &INPUT_KEY_UP.to_le_bytes());
        assert_eq!(&messages[0].bytes[20..24], &[0, b'A', 0, 0]);
    }

    #[test]
    fn typed_email_and_pasted_email_preserve_every_character() {
        let email = b"a.b@gmail.com";
        let mut pasted = vec![0x22, 23, 0, 0, 0];
        pasted.extend_from_slice(email);
        let mut codec = NvstInputCodec::default();

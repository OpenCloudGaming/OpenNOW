...
const MAX_QUEUED_INPUT_PACKETS: usize = 128;

/// Released on processing, send failure, or receiver shutdown, so the nonblocking
/// capture path stays bounded even when the network worker stalls.
struct InputQueuePermit(Arc<AtomicUsize>);

impl InputQueuePermit {
    fn acquire(pending: &Arc<AtomicUsize>) -> Result<Self, TransportError> {
        pending
            .fetch_update(Ordering::AcqRel, Ordering::Acquire, |count| {
                (count < MAX_QUEUED_INPUT_PACKETS).then_some(count + 1)
            })
            .map_err(|_| TransportError::InputBackpressured)?;
        Ok(Self(Arc::clone(pending)))
    }
}
...
enum UdpReceiverCommand {
    ...
    SendInput {
        bytes: Vec<u8>,
        queued_at: Instant,
        _permit: Option<InputQueuePermit>,
...
impl NvstUdpReceiverControl {
    /// One capture producer waits for acknowledgement; timeout stops that producer, so
    /// audio cannot build an unbounded backlog behind a stalled receive worker.
    pub fn send_microphone(
        &self,
        payload: Vec<u8>,
        timestamp: u32,
        marker: bool,
        ...
        if !self.input_ready.load(Ordering::Acquire) || payload.is_empty() || payload.len() > 4096 {
            return false;
        }
        let (reply, result) = mpsc::sync_channel(1);
        if self
            .commands
            .send(UdpReceiverCommand::Microphone {
                payload,
                timestamp,
                marker,
    ...
    pub fn queue_input(
        ...
        let permit = InputQueuePermit::acquire(&self.pending_input)?;
        self.commands
            .send(UdpReceiverCommand::SendInput {
                bytes,
                queued_at: Instant::now(),
...
fn run_nvst_webrtc_bundle(
    ...
    // The native-bundle microphone uses the same mic MID as GFN's WebRTC offer.
    // str0m owns SRTP, sequence tracking, RTCP and socket output.
    let microphone_ssrc = {
        let mut api = rtc.direct_api();
        let mid = Mid::from("3");
    ...
    loop {
        // Bound command draining so a high-polling mouse cannot starve network I/O.
        for _ in 0..32 {
            match commands.try_recv() {
                ...
                Ok(UdpReceiverCommand::Microphone {
                    payload,
                    timestamp,
                    marker,
                    captured_at,
                    ...
                    let sent = dtls_ready
                        ...
                        && captured_at.elapsed() < Duration::from_millis(100)
                        && input_state.is_ready();
                    if sent {
                        if let Some(stream) = rtc.direct_api().stream_tx(&microphone_ssrc) {
                            stream.write_rtp(
                            ...
                            microphone_sequence = microphone_sequence.wrapping_add(1);
                ...
                Ok(UdpReceiverCommand::SendInput {
                    ...
                    input_queue_max_us = input_queue_max_us.max(queued_at.elapsed().as_micros());
                    input_packets += 1;
                    let mut sent = true;
        ...
        if now.duration_since(last_control_stats_log) >= Duration::from_secs(2) {
            ...
            if input_packets > 0 {
                opennow_streamer_protocol::log::log_async(
                    "INFO",
                    "nvst-input",
                    &format!("packets={input_packets} max_queue_us={input_queue_max_us}"),
                );
                input_packets = 0;
                input_queue_max_us = 0;
            }
            ...
            match rtc.poll_output() {
                ...
                Ok(Output::Event(event)) => match event {
                    ...
                    Event::ChannelData(data) => {
                        ...
                        {
                            // Only input/control channels may carry haptics. Cursor/image
                            // payloads never enter the platform input parser.
                            if data.id != channels.cursor && data.data.len() <= 4096 {
                                let _ = event_sender
                                    .send(NvstReceiveEvent::ServerInput(data.data.to_vec()));
                            }
                            ...
                            for message in cursor_messages {
                                ...
                                if let Some(cursor) = message.normalized {
                                        ...
                                        && channels.send_mouse_cursor_capture(&mut rtc, true)
                                    {
                                        ...
                                        eprintln!(
                                            "NVST cursor capture tx: channel={} command=0x0308 enabled=true reason=android-server-cursor",
                                            channels.label(channels.control_reliable),
                                        );
                                    }
                                    eprintln!(
    ...
    loop {
        loop {
            match commands.try_recv() {
                ...
                Ok(UdpReceiverCommand::Microphone { reply, .. }) => {
                    let _ = reply.send(false);
                }
...
mod tests {
    ...
    #[test]
    fn queued_input_is_nonblocking_ordered_and_bounded() {
        let (commands, receiver) = mpsc::channel();
        let pending = Arc::new(AtomicUsize::new(0));
        let control = NvstUdpReceiverControl {
            commands,
            input_ready: Arc::new(AtomicBool::new(true)),
            pending_input: pending.clone(),
        };
        // No receive worker is running: these calls must return without waiting for an ACK.
        for index in 0..MAX_QUEUED_INPUT_PACKETS {
            control.queue_input(vec![index as u8], false).unwrap();
        }
        assert!(matches!(
            control.queue_input(vec![0], false),
            Err(TransportError::InputBackpressured)
        ));
        assert_eq!(pending.load(Ordering::Acquire), MAX_QUEUED_INPUT_PACKETS);
        for index in 0..MAX_QUEUED_INPUT_PACKETS {
            match receiver.try_recv().unwrap() {
                UdpReceiverCommand::SendInput { bytes, reply, .. } => {
        ...
        control.queue_input(vec![42], false).unwrap();
        drop(receiver);
        assert_eq!(pending.load(Ordering::Acquire), 0);
        assert!(matches!(
            control.queue_input(vec![43], false),
    ...
    #[test]
    fn queued_input_rejects_before_handshake_without_consuming_capacity() {
        ...
        assert!(matches!(
            control.queue_input(vec![7], false),
            Err(TransportError::InputNotReady)
        ));
        assert_eq!(pending.load(Ordering::Acquire), 0);
    }

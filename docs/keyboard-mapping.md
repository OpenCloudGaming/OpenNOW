...
The Windows client continues to use native virtual keys. The macOS logical-key
path is unchanged. Unknown Linux layouts and synthetic events without a native
scan code retain logical-key fallback. Layout lookup occurs when the property

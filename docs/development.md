...
GitHub Actions also builds a testable unsigned Android debug APK artifact in the `auto-build` workflow for pull requests, pushes to `main` and `dev`, plus manual workflow runs.
